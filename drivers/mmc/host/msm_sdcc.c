...
{
	mb();
	udelay(host->reg_write_delay);      //CR309435 109804
}
...
static void msmsdcc_do_cmdirq(struct msmsdcc_host *host, uint32_t status)
{
	...
	//ruanmeisi_20100510
	//del_timer(&host->command_timer);   jzq
	//end
...
msmsdcc_irq(int irq, void *dev_id)
{
	...
	do {
		...
		if (!host->clks_on) {
			...
			if (host->clk_rate <=msmsdcc_get_min_sup_clk_rate(host))
				msmsdcc_delay(host);			                  //CR309435 109804
		}
...
msmsdcc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	//printk(KERN_ERR "%s: jzq set clock %u\n",mmc_hostname(host->mmc), ios->clock);   25000000
			...
			host->reg_write_delay =
				(1 + ((3 * USEC_PER_SEC) /
				          (host->clk_rate ? host->clk_rate :
				          	msmsdcc_get_min_sup_clk_rate(host))));		 //CR309435 109804	
		}
		/*
...
static ssize_t
show_clk(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mmc_host *mmc = dev_get_drvdata(dev);
	struct msmsdcc_host *host = mmc_priv(mmc);
...
static DEVICE_ATTR(clk, S_IRUGO | S_IWUSR,
		show_clk, NULL);
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if (plat->sdiowakeup_irq) {
		...
		ret = request_irq(plat->sdiowakeup_irq,
			...
			DRIVER_NAME "wowgpiowakeup", host);
...
static int msmsdcc_remove(struct platform_device *pdev)
{
	...
	//end
	//ruanmeisi_20100510
	//del_timer_sync(&host->command_timer);     jzq
	//end
	if (!plat->status_irq)
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
     ...
     #ifdef CONFIG_ZTE_MMC_PATCH	
      	   #ifdef CONFIG_BCM_WIFI       //shaohua add for wifi suspend
		if (!mmc->card || mmc->card->type != MMC_TYPE_SDIO || host->pdev_id != 2){            
             		/* If there is pending detect work abort runtime suspend */
               	if (unlikely(work_busy(&mmc->detect.work)))      //jzq fix deadlock in mmc_suspend_host and mmc_rescan 20120224
                       	rc = -EAGAIN;
               	else		
