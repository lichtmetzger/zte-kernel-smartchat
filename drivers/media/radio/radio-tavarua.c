...
static int tavarua_fops_release(struct file *file)
{
	...
	radio->pdata->fm_shutdown(radio->pdata);
	if (radio->pdata->config_i2s_gpio != NULL){
		printk(KERN_WARNING DRIVER_NAME ": tavarua_fops_release  FM_I2S_OFF \n");
		radio->pdata->config_i2s_gpio(FM_I2S_OFF);
	}
	radio->handle_irq = 1;
		...
		struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_PRIVATE_TAVARUA_SRCHON:
		...
		break;
	case V4L2_CID_PRIVATE_TAVARUA_STATE:
		printk(KERN_ERR "%s: V4L2_CID_PRIVATE_TAVARUA_STATE set handle_irq 1\n",
					__func__);	
		/* check if already on */
		radio->handle_irq = 1;
					...
					enum radio_state_t state)
{
	...
	if (!radio->lp_mode){//songyy
			printk(KERN_WARNING DRIVER_NAME ": tavarua_setup_interrupts  !radio->lp_mode \n");
