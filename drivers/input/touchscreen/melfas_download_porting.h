...
#define TKEY_VDD_SET_HIGH() gpio_direction_output(GPIO_VDD, 1);\
							printk("GPIO_VDD(%d) = %d==1\n", GPIO_VDD, gpio_get_value(GPIO_VDD))
#define TKEY_VDD_SET_LOW()  gpio_direction_output(GPIO_VDD, 0);\
							printk("GPIO_VDD(%d) = %d==0\n", GPIO_VDD, gpio_get_value(GPIO_VDD))
#else
...
#define TKEY_INTR_SET_HIGH() gpio_direction_output(GPIO_INTR, 1)
#define TKEY_INTR_SET_LOW()  gpio_direction_output(GPIO_INTR, 0)
#define TKEY_INTR_SET_OUTPUT()  
#define TKEY_INTR_SET_INPUT()  gpio_direction_input(GPIO_INTR)
#else
...
#define TKEY_RESETB_SET_HIGH(vreg_s3)    vreg_enable(vreg_s3);\
										printk("TKEY_RESETB_SET_HIGH\n")
#define TKEY_RESETB_SET_LOW(vreg_s3)     vreg_disable(vreg_s3);\
										printk("TKEY_RESETB_SET_HIGH\n")
#define TKEY_RESETB_SET_OUTPUT()
#define TKEY_RESETB_SET_INPUT() 
#else
#define TKEY_RESETB_SET_HIGH(vreg_s3)            											// Nothing
#define TKEY_RESETB_SET_LOW(vreg_s3)          												// Nothing
...
#define TKEY_I2C_SCL_SET_HIGH()	gpio_direction_output(GPIO_I2C_SCL, 1)
#define TKEY_I2C_SCL_SET_LOW()	gpio_direction_output(GPIO_I2C_SCL, 0)

#define TKEY_I2C_SDA_SET_HIGH()	gpio_direction_output(GPIO_I2C_SDA, 1);
#define TKEY_I2C_SDA_SET_LOW()	gpio_direction_output(GPIO_I2C_SDA, 0);

#define TKEY_I2C_SCL_SET_OUTPUT()
