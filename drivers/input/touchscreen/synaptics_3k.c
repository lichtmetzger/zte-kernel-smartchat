...
#elif defined (CONFIG_MACH_BLADE2)//P772A30   800*480      MENU HOME BACK SEARCH 
static const char ts_keys_size[] = "0x01:139:36:850:60:60:0x01:102:174:850:60:60:0x01:158:308:850:60:60:0x01:217:437:850:60:60";
...
#elif defined (CONFIG_MACH_ROAMER2)||defined (CONFIG_MACH_DEEPBLUE)//N788  320*480       MENU HOME BACK SEARCH
static const char ts_keys_size[] = "0x01:139:45:510:50:40:0x01:102:121:510:60:40:0x01:158:198:510:60:40:0x01:217:273:510:60:40";
#else
...
#if defined(CONFIG_MACH_HOPE)||defined(CONFIG_MACH_ATLAS40)||defined(CONFIG_MACH_ROAMER2)||defined(CONFIG_MACH_DEEPBLUE)
static int
			  ...
			  void *data)
{
	...
	if (ret < 0){
		...
		//len += sprintf(page + len, "config ID : %c%c%c%c\n", 
		...
		case '1':
			len += sprintf(page + len, "chip type : S2200 (0x%x)\n",  synaptics_config_id[0]);
			break;
		case '2':
			len += sprintf(page + len, "chip type : S2202 (0x%x)\n",  synaptics_config_id[0]);
			break;
		case '3':
			len += sprintf(page + len, "chip type : S3200 (0x%x)\n",  synaptics_config_id[0]);
			break;
		case '4':
			len += sprintf(page + len, "chip type : S3202 (0x%x)\n",  synaptics_config_id[0]);
			break;
		case '5':
			len += sprintf(page + len, "chip type : S3203 (0x%x)\n",  synaptics_config_id[0]);
			break;
		case '6':
			len += sprintf(page + len, "chip type : S7020 (0x%x)\n",  synaptics_config_id[0]);
			break;
		case '7':
			len += sprintf(page + len, "chip type : S7300 (0x%x)\n",  synaptics_config_id[0]);
			break;
		default:
			len += sprintf(page + len, "chip type : unknown (0x%x)\n",  synaptics_config_id[0]);
		}

		switch (synaptics_config_id[1]){//sensor partner
		case '1':
			len += sprintf(page + len, "sensor partner : TPK (0x%x) \n", synaptics_config_id[1] );
			break;
		case '2':
			len += sprintf(page + len, "sensor partner : Truly (0x%x) \n", synaptics_config_id[1] );
			break;
		case '3':
			len += sprintf(page + len, "sensor partner : Success (0x%x) \n", synaptics_config_id[1] );
			break;
		case '4':
			len += sprintf(page + len, "sensor partner : Ofilm (0x%x) \n", synaptics_config_id[1] );
			break;
		case '5':
			len += sprintf(page + len, "sensor partner : Lead (0x%x) \n", synaptics_config_id[1] );
			break;
		case '6':
			len += sprintf(page + len, "sensor partner : Wintek (0x%x) \n", synaptics_config_id[1] );
			break;
		case '7':
			len += sprintf(page + len, "sensor partner : Laibao (0x%x) \n", synaptics_config_id[1] );
			break;
		case '8':
			len += sprintf(page + len, "sensor partner : CMI (0x%x) \n", synaptics_config_id[1] );
			break;
		case '9':
			len += sprintf(page + len, "sensor partner : ECW (0x%x) \n", synaptics_config_id[1] );
			break;
		case 'A':
			len += sprintf(page + len, "sensor partner : Goworld (0x%x) \n", synaptics_config_id[1] );
			break;
		default:
			len += sprintf(page + len, "sensor partner : unknown (0x%x) \n", synaptics_config_id[1] );
		}
	...
	if ( ts->f11.flag && (interrupt & ts->f11.intr_mask) ) 
	{
		...
		if (will_report) {
			for_each_set_bit(i, pointers_to_report, NR_FINGERS) {
    			...
    			if(new_report_data[0].y >=1785)//LCD 820
    			{
    				input_report_key(ts->input_dev, BTN_TOUCH, !!new_report_data[0].z);// recovery mode use
    			}
    #elif defined (CONFIG_MACH_BLADE2)//XYM FOR P772A30
    			if(new_report_data[0].y >=1562)//LCD 820
    			{
    				input_report_key(ts->input_dev, BTN_TOUCH, !!new_report_data[0].z);// recovery mode use
    			}
    #elif defined (CONFIG_MACH_HOPE)//N855
			if(new_report_data[0].y >=1736)//LCD 500    1667/480*500=1736
			{
				input_report_key(ts->input_dev, BTN_TOUCH, !!new_report_data[0].z);// recovery mode use
			}
    #elif defined (CONFIG_MACH_ROAMER2) ||defined(CONFIG_MACH_DEEPBLUE) //N788 N790
			if(new_report_data[0].y >=1520)//
			{
				input_report_key(ts->input_dev, BTN_TOUCH, !!new_report_data[0].z);// recovery mode use
			}
    #endif	
...
#if 0
{
#if defined (CONFIG_MACH_BLADE2) && !defined(CONFIG_TOUCHSCREEN_VIRTUAL_KEYS)

	char buf=0x0;
	buf = i2c_smbus_read_byte_data(ts->client, ts->f11.ctrl_base+0);
	printk("xym---read buf=0x%x\n",buf);
	buf=(buf|0x1)&0xf9;
	printk("xym---write buf=0x%x\n",buf);
	ret = i2c_smbus_write_byte_data(ts->client, ts->f11.ctrl_base+0, buf);	
	if (ret<0) pr_err("%s: i2c_smbus_write_byte_data failed\n",__func__);
	
		printk("xym---write 0x07 to ts->f11.ctrl_base+01\n");
	ret = i2c_smbus_write_byte_data(ts->client, ts->f11.ctrl_base+01, 0x07);
	if (ret<0) pr_err("%s: i2c_smbus_write_byte_data failed\n",__func__);
	
	printk("xym---write 0x02 to ts->f11.ctrl_base+02\n");
ret = i2c_smbus_write_byte_data(ts->client, ts->f11.ctrl_base+02, 0x02);
if (ret<0) pr_err("%s: i2c_smbus_write_byte_data failed\n",__func__);

printk("xym---write 0x02 to ts->f11.ctrl_base+03\n");
ret = i2c_smbus_write_byte_data(ts->client, ts->f11.ctrl_base+03, 0x02);
if (ret<0) pr_err("%s: i2c_smbus_write_byte_data failed\n",__func__);
#endif
}
#endif
    ...
    #if defined(CONFIG_MACH_BLADE2)
    	max_y = 1524;//实际测量得到值//XYM FOR P772A30
    #elif defined (CONFIG_MACH_ATLAS40)//P740A  800*480       MENU HOME BACK SEARCH 
    	max_y = 1747-6;
    #elif defined (CONFIG_MACH_HOPE)//N855       MENU HOME BACK SEARCH 
    	max_y = 1667;//多次调试得到值
    #elif defined (CONFIG_MACH_ROAMER2)||defined(CONFIG_MACH_DEEPBLUE)//N788 N790	 MENU HOME BACK SEARCH 
    	max_y = 1500;//1515去掉虚拟按键的部分
    #else
    	max_y = ts->max[1];
    #endif
...
#ifdef TOUCHSCREEN_DUPLICATED_FILTER
{
	...
	ts->dup_threshold= (2*max_y)/yres;//xym temp
