...
int oops=0;
static void dops(void)
{
        struct task_struct *g, *p;
        unsigned state;
        static const char stat_nam[] = "RSDTtZX";

        printk("pid   ppid  prio task            pc\n");
        read_lock(&tasklist_lock);
        do_each_thread(g, p) {
                state = p->state ? __ffs(p->state) + 1 : 0;
                printk("%5d %5d %4d ", p->pid, p->parent->pid, p->prio);
                printk("%-13.13s %c", p->comm,
                        state >= sizeof(stat_nam) ? '?' : stat_nam[state]);
                if (state == TASK_RUNNING)
                        printk(" running\n");
                else
                        printk(" %08lx,%08lx\n", thread_saved_pc(p),KSTK_EIP(p));
...
static void dobacktrace(void)
{
        struct task_struct *g, *p;
        read_lock(&tasklist_lock);
        do_each_thread(g, p) {
		if (p->pid<250){
        	printk("dobacktrace---%s\n", p->comm);
